...
best_rules = rules
# Initialise a variable to set the rule form (with or without a conversion
# point
rule_form = 0
# Generate the initial Sum of Square Residuals (SSR) values

dummy = ef_ssr(log_data_ef, log_sims_ef, att_rules, luc, act)
ssr_matrix = dummy[0]
ssr_sum = dummy[1]
error_0 = dummy[2]
...
while ssr_sum < ssr_sum_old:
    ...
    ratio = float(adjust_rule_values[2]) / float(adjust_rule_values[1])
    # Determine whether to include conversion point in adjustment.
    # The fine tuning includes the adjustment of a conversion point.
    if error_0[lu_elem, fu_elem] > 0:
        rule_form = 0
        ...
        y0_low = min_conversion
        y1_low = min_y1_influence
        y2_low = min_y1_influence*ratio
        # Set the rule in the project file.
        set_lp_rule(project_file, fu_elem, lu_elem, y0_low, y1_low, y2_low,
                    xe)
        # Calculate the enrichment for the low influence settings,
        # then log scale.
        low_sims_ef = multi_sims_ef(max_runs, base_seed, smap_path,
                                    project_file, log_file, working_directory,
                                    geo_cmd, luc, max_distance, cdl, cd, N,
                                    omap, mask, rows, cols)
        log_low_sims_ef = log_scale_ef(low_sims_ef, log_base, luc, act, pas,
                                       max_distance)
        # Calculate the global SSR for the low influence settings.
        low_ssr_output = ef_ssr(log_data_ef, log_low_sims_ef, att_rules,
                                luc, act)
        ssr_low = low_ssr_output[1]
        conversions_low = low_ssr_output[3]
        # Specify the high influence values for the required distances.
        y0_high = max_conversion
        y1_high = max_y1_influence
        y2_high = max_y1_influence*ratio
        # Set the rule in the project file.
        set_lp_rule(project_file, fu_elem, lu_elem, y0_high, y1_high, y2_high,
                    xe)
        # Calculate the enrichment for the high influence settings,
        # then log scale.
        high_sims_ef = multi_sims_ef(max_runs, base_seed, smap_path,
                                     project_file, log_file,
                                     working_directory, geo_cmd, luc,
                                     max_distance, cdl, cd, N, omap,
                                     mask, rows, cols)
        log_high_sims_ef = log_scale_ef(high_sims_ef, log_base, luc, act, pas,
                                       max_distance)
        # Calculate the global SSR for the high influence settings.
        high_ssr_output = ef_ssr(log_data_ef, log_high_sims_ef, att_rules,
                                 luc, act)
        ssr_high = high_ssr_output[1]
        conversions_high = high_ssr_output[3]
        # Initialise the bracketing influence values, low (a) and high (b).
        y0_a = min_conversion
        y1_a = min_y1_influence
        y2_a = min_y1_influence*ratio
        y0_b = max_conversion
        y1_b = max_y1_influence
        y2_b = max_y1_influence*ratio

        # Stage 2. Apply the Golden Section Search to converge on the
        # best influence values
        while (abs(y1_a - y1_b) > gss_tol):
            # Calculate the interval influence values.
            y0_c = y0_b - (y0_b - y0_a)/gr
            y0_d = y0_a + (y0_b - y0_a)/gr
            y1_c = y1_b - (y1_b - y1_a)/gr
            y1_d = y1_a + (y1_b - y1_a)/gr
            y2_c = y2_b - (y2_b - y2_a)/gr
            y2_d = y2_a + (y2_b - y2_a)/gr
            # Set the neighbourhood rule to the lower interval values.
            set_lp_rule(project_file, fu_elem, lu_elem, y0_c, y1_c, y2_c, xe)
            ...
            ssr_c = ef_ssr(log_data_ef, log_c_sims_ef, att_rules, luc, act)
            f_c = ssr_c[1]
            f_c_convo = ssr_c[3]
            # Set the neighbourhood rule to the higher interval values.
            set_lp_rule(project_file, fu_elem, lu_elem, y0_d, y1_d, y2_d, xe)
            # Calculate the simulated enrichment factors, log scale.
            ...
            ssr_d = ef_ssr(log_data_ef, log_d_sims_ef, att_rules, luc, act)
            f_d = ssr_d[1]
            f_d_convo = ssr_d[3]
            # Print output
            print("Current conversion bracket: " + str(y0_c) + "|" +
                  str(y0_d))
            print("Current influence bracket: " + str(y1_c) + "|" +
                  str(y1_d))
            print("SSR conversion bracket: " + str(f_c_convo) + "|" +
                   str(f_d_convo))
            print("SSR influence bracket: " + str(f_c) + "|" + str(f_d))
            # Evaluate the result
            if f_c_convo < f_d_convo:
                y0_b = y0_d
            else:
                y0_a = y0_c
            if f_c < f_d:
    ...
    # The fine tuning doesn't include the adjustment of a conversion point.
    else:
        rule_form = 1
        # Stage 1. Set to extreme values and initialise the GSS
        # Set fixed elements for the neighbourhood rule adjustment.
        ...
        y2_low = min_y1_influence*ratio
        # Set the rule in the project file
        set_lp_rule(project_file, fu_elem, lu_elem, y0, y1_low, y2_low, xe)
        # Calculate the enrichment for the low influence settings,
        # then log scale.
        low_sims_ef = multi_sims_ef(max_runs, base_seed, smap_path,
                                    project_file, log_file, working_directory,
        ...
        y2_high = max_y1_influence * ratio
        # Set the rule in the project file.
        set_lp_rule(project_file, fu_elem, lu_elem, y0, y1_high, y2_high, xe)
        # Calculate the enrichment for the high influence settings,
        # then log scale.
        high_sims_ef = multi_sims_ef(max_runs, base_seed, smap_path,
                                     project_file, log_file,
    ...
    if rule_form == 1:
        ...
        if f_c_convo <= f_d_convo:
            if f_c <= f_d:
                rules[rule_name] = [y0_c, y1_c, y2_c, xe]
            else:
                rules[rule_name] = [y0_c, y1_d, y2_d, xe]
        else:
            if f_c <= f_d:
                rules[rule_name] = [y0_d, y1_c, y2_c, xe]
            else:
                rules[rule_name] = [y0_d, y1_d, y2_d, xe]
    ...
    sims_ef = multi_sims_ef(max_runs, base_seed, smap_path, project_file,
                            ...
                            max_distance, cdl, cd, N, omap, mask, rows, cols)
    log_sims_ef = log_scale_ef(sims_ef, log_base, luc, act, pas, max_distance)
    ssr_output = ef_ssr(log_data_ef, log_sims_ef, att_rules, luc, act)
