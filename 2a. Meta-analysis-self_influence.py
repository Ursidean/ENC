...
# Start with the self-influence tail values. First, specify the range
# that will be tested.
varied_parameter = "theta_si"
low_bound = 0
high_bound = 51
theta_si_step = 0.01
for a in range(low_bound, high_bound, 1):
    theta_si1 = float(a)*theta_si_step
    theta_si2 = theta_si1*0.1
    # Set the requisite parameter value
    # Set the distance 1 tail values for self-influence rules.
    d1_high_si_value = high_inertia * theta_si1
    d1_mid_si_value = mid_inertia * theta_si1
    d1_low_si_value = low_inertia * theta_si1
    ...
    for i in range(0, act):
        for j in range(0, luc):
            ...
            if i + pas == j:
                ...
                # Skip if not a self-influence rule.
                pass
                # Input the rules into the model.
    # Input the rules to the specified project files.
    # This is only done for the self-influence rules.
    for i in range(0, luc):
        for j in range(0, act):
            key = "from " + luc_names[i] + " to " + luc_names[j + pas]
            fu_elem = j
...
metrics_output_file = output_path + case_study + "\\theta_si_meta_analysis_output.csv"
store = [0]*5
with open (metrics_output_file, "wb") as csv_file:
    writer = csv.writer(csv_file)
    values = ["theta_si value", "KSIM avg.", "KSIM_err", "CLU avg", "CLU_err"]
    writer.writerow(values)
    for a in range(low_bound, high_bound, 1):
        store[0] = float(a)*theta_si_step
