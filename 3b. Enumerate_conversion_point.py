...
# Use pre-set values for other parameter values.
theta_cp = 0.025
theta_c1 = 0.005
...
# Calculate the fixed components.
...
# Second, enumerate across the range of ratio values for conversion point.
print "Enumerating conversion point values"
# Use pre-set values for other parameter values.
theta_i1 = 0.05
theta_i2 = theta_i1 * 0.1
theta_c1 = 0.005
theta_c2 = theta_c1 * 0.1
# Calculate the set parameter values.
# Set the distance 1 tail values for self-influence rules.
d1_high_si_value = high_inertia * theta_i1
d1_mid_si_value = mid_inertia * theta_i1
d1_low_si_value = low_inertia * theta_i1
...
d2_mid_co_value = mid_inertia * theta_c2
d2_low_co_value = low_inertia * theta_c2
# Perform the iteration across the values.

for b in range(low_bound, high_bound, 1):
    # Set the variable parameter value. This will change depending on
    # what is tested. Hard coded for convenience.
    varied_parameter = "theta_cp"
    theta_cp = float(b)*theta_cp_step
    print theta_cp
    # Calculate the relevant parameter values
    high_conversion = high_inertia * theta_cp
    mid_conversion = mid_inertia * theta_cp
...
# Finally, enumerate across the range of ratio values for conversion tail.
print "Enumerating theta conversion tail values"
# Use pre-set values for other parameter values.
theta_cp = 0.025
theta_i1 = 0.05
theta_i2 = theta_c1 * 0.1
# Calculate the relevant parameter values
# Set the distance 1 tail values for self-influence rules.
d1_high_si_value = high_inertia * theta_i1
d1_mid_si_value = mid_inertia * theta_i1
...
for b in range(low_bound, high_bound, 1):
    ...
    theta_c1 = float(b)*theta_ct_step
    theta_c2 = 0.1 * theta_i1
    print theta_c1
    # Set the distance 1 tail values for interaction rules.
    d1_high_co_value = high_inertia * theta_c1
    d1_mid_co_value = mid_inertia * theta_c1
...
metrics_output_file = (
    output_path + case_study + "meta_parameter_sa.csv"
)
...
with open (metrics_output_file, "wb") as csv_file:
    ...
    # Second, write the values for conversion point values.
    values = ["Var Par Value", "Seed", "KSIM", "AWACE"]
    writer.writerow(values)
    # Iterate through dictionary to write.
    for b in range(low_bound, high_bound, 1):
        varied_parameter = "theta_cp"
        theta_cp = float(b) * theta_cp_step
        store[0] = str(theta_cp)
        for x in range(0, max_runs):
            seed = base_seed + x
            store[1] = str(x)
            key1 = (
            ...
            store[3] = metrics[key2]
            writer.writerow(store)
    # Finally, write the values for conversion tail values.
    values = ["Var Par Value", "Seed", "KSIM", "AWACE"]
    writer.writerow(values)
    # Iterate through dictionary to write.
    for b in range(low_bound, high_bound, 1):
        varied_parameter = "theta_ct"
        theta_ct = float(b) * theta_ct_step
        store[0] = str(theta_cp)
        for x in range(0, max_runs):
            seed = base_seed + x
            store[1] = str(x)
