...
# Start with the self-influence tail values. First, specify the range that
# will be tested.
varied_parameter = "theta_cp"
low_bound = 0
high_bound = 201
theta_cp_step = 0.0005
for a in range(low_bound, high_bound, 10):
    theta_cp = float(a)*theta_cp_step
    # Set the requisite conversion parameter value
    high_conversion = high_inertia * theta_cp
    mid_conversion = mid_inertia * theta_cp
    low_conversion = low_inertia * theta_cp
...
metrics_output_file = output_path + case_study + "\\theta_cp_meta_analysis_output.csv"
store = [0]*5
with open (metrics_output_file, "wb") as csv_file:
    writer = csv.writer(csv_file)
    values = ["theta_cp value", "KSIM avg.", "KSIM_err", "CLU avg", "CLU_err"]
    writer.writerow(values)
    for a in range(low_bound, high_bound, 10):
        store[0] = float(a)*theta_cp_step
